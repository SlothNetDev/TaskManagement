        ...
        IAuthService
    {
        public async Task<ResponseType<string>> RegisterAsync(RegisterRequestDto registerDto)
        {
            ...
            var user = new ApplicationUsers
            {
                ...
                DomainUser =
                    domainUser // Ensure DomainUser is configured to be saved with ApplicationUser or separately
            };
            ...
            if (!result.Succeeded)
            {
                ...
                _logger.LogWarning(
                    "REG_003: Error occurred when creating user through UserManager for {Email}. Errors: {@Errors}",
           ...
           await _dbContext.SaveChangesAsync();
            ...
            _logger.LogInformation("REG_005: User {Email} successfully registered with role '{Role}'.", user.Email,
                role);
            return ResponseType<string>.SuccessResult(
                 $"{user.UserName} Successfully registered with role '{role}'.",
        ...
        public async Task<ResponseType<AuthResultDto>> LoginAsync(LoginRequestDto loginDto)
        {
            ...
            _logger.LogInformation(
                "LOG_004: User {UserId} (Email: {Email}) logged in successfully with role: {UserRole}", user.Id,
                user.Email, userRole);
